    ...
    <div className="member-page-flow">
      ...
      <section aria-labelledby="incoming-collaborations">
        ...
        {incoming.length === 0 ? <div className="member-empty">{text.noIncomingCollaborations}</div> : <div className="member-content-grid">{incoming.map((item) => (
          <button className="member-content-card" type="button" key={item.id} onClick={() => onOpen({ source: 'collaboration', collaborationId: item.id, path: '.' }, labelFor(item), item.permission === 'viewer')}>
      ...
      <section aria-labelledby="outgoing-collaborations">
        ...
        {outgoing.length === 0 ? <div className="member-empty">{text.noOutgoingCollaborations}</div> : <div className="member-content-grid">{outgoing.map((item) => (
          <article className="member-content-card" key={item.id}>
            <FileTypeIcon kind="dir" name={labelFor(item)} className="member-file-icon dir" />
            <strong>{labelFor(item)}</strong>
