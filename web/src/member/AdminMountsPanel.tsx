...
export default function AdminMountsPanel({ locale, isInitialAdmin }: { locale: MemberLocale; isInitialAdmin: boolean }) {
  ...
  async function onCreate(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice('');
    try {
      await registerAdminMount({ ...form, governance: isInitialAdmin ? form.governance : 'normal' });
  ...
  async function onSaveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice('');
    try {
      await updateAdminMount(selectedMount.id, {
  ...
  async function onAddGrant(event: FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      const grant = await putAdminMountGrant(selectedMount.id, grantAccountId, grantPermission);
  ...
  async function onRemoveGrant(grant: AdminMountGrant) {
    ...
    setError('');
    try {
      await deleteAdminMountGrant(selectedMount.id, grant.accountId);
  ...
  async function onDelete(event: FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      await deleteAdminMount(deleteTarget.id, deleteConfirmation);
