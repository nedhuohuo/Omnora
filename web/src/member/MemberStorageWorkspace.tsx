...
export default function MemberStorageWorkspace({ locale, view }: Props) {
  ...
  async function createFolder() {
    ...
    setError('');
    try {
      await createMemberDirectory(activeSource.locator, name.trim());
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      for (let part = 1; part <= totalParts; part += 1) {
        ...
        await fetch(`/api/v1/member/uploads/${encodeURIComponent(uploadID)}/parts/${part}`, {
          method: 'PUT',
    ...
    return (
      <div className="member-page-flow">
        <div className="member-crumbs">
          <button type="button" onClick={() => { setActiveSource(null); setEntries([]); setPath('.'); setShowTrash(false); }}>{view === 'files' ? text.myFiles : text.collaboration}</button>
        ...
        <div className="member-toolbar">
          ...
          <button type="button" onClick={() => void loadTrash(activeSource)}>{text.trashTitle}</button>
          <span className="member-toolbar-spacer" />
          <input aria-label={text.search} value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void search(); }} placeholder={text.search} />
