...
type MemberTab = 'files' | 'collaborations' | 'shares' | 'tokens' | 'docs' | 'account';
...
export default function MemberFilesApp({ entry = 'member' }: Props) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        } catch { setInitializationAvailable(false); }
      }
    })();
  }, []);
  ...
  async function onLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setIsAdmin(session.isAdmin === true); setIsInitialAdmin(session.isInitialAdmin === true);
  ...
  async function beginEnrollment() {
    ...
    try { const response = await setupTOTP(); setEnrollment({ secret: response.secret ?? '', otpauthUri: response.otpauthUri }); }
    catch (caught) { setError(describeError(caught)); } finally { setLoading(false); }
  }
  ...
  async function onConfirmEnrollment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const session = await getSession();
      setIsAdmin(session.isAdmin === true); setIsInitialAdmin(session.isInitialAdmin === true);
      setEnrollment(null); setEnrollmentCode(''); setSessionState(stateForSession(session));
    } catch (caught) { setError(describeError(caught)); } finally { setLoading(false); }
  }

  async function onInitialize(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setLoading(true); setError(''); setSetupNotice('');
    try {
      await initialize(setupForm);
      setSetupNotice(text.setupComplete); setSetupMode(false); setInitializationAvailable(false);
  ...
  async function onLogout() {
    ...
    setSessionState('signed-out'); setIsAdmin(false); setIsInitialAdmin(false); setEnrollment(null);
  }
  ...
  const activeAdminGroup = typeof activeTab === 'string' && ['overview', 'users', 'mounts', 'index-jobs', 'route-groups', 'share-governance', 'token-governance', 'audit', 'backups'].includes(activeTab) ? adminGroupForTab(activeTab as AdminTab) : 'overview';
  const adminView = isAdmin && (entry === 'admin' || typeof activeTab !== 'string' || activeTab === 'overview' || activeTab === 'users' || activeTab === 'mounts' || activeTab === 'index-jobs' || activeTab === 'route-groups' || activeTab === 'share-governance' || activeTab === 'token-governance' || activeTab === 'backups' || activeTab === 'audit');

  return <main className="member-app-shell"><header className="member-topbar"><strong>{entry === 'admin' ? text.adminAccessSecurity : text.signIn}</strong><div className="member-topbar-actions"><button type="button" onClick={() => setLocale(locale === 'zh-CN' ? 'en-US' : 'zh-CN')}>{locale === 'zh-CN' ? 'EN' : '中文'}</button><button type="button" onClick={() => void onLogout()}>{text.signOut}</button></div></header><div className="member-layout"><aside className="member-sidebar">
    {entry === 'member' && <><button className={`member-nav ${activeTab === 'files' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('files')}>{text.myFiles}</button><button className={`member-nav ${activeTab === 'collaborations' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('collaborations')}>{text.collaboration}</button><button className={`member-nav ${activeTab === 'shares' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('shares')}>{text.navShares}</button><button className={`member-nav ${activeTab === 'tokens' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('tokens')}>{text.navTokens}</button><button className={`member-nav ${activeTab === 'docs' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('docs')}>{text.navDocs}</button><button className={`member-nav ${activeTab === 'account' ? 'active' : ''}`} type="button" onClick={() => setActiveTab('account')}>{text.account}</button></>}
    {isAdmin && <>{adminItems.map((group) => <div className="member-sidebar-section" key={group.id}><p>{group.label}</p>{group.tabs.map((item) => <button className={`member-nav ${activeTab === item.id ? 'active' : ''}`} type="button" onClick={() => setActiveTab(item.id)} key={item.id}>{item.label}</button>)}</div>)}</>}
  </aside><section className="member-main-content">{adminView ? <AdminWorkspace tab={activeTab as AdminTab} locale={locale} isInitialAdmin={isInitialAdmin} /> : activeTab === 'files' ? <MemberStorageWorkspace locale={locale} view="files" /> : activeTab === 'collaborations' ? <MemberStorageWorkspace locale={locale} view="collaborations" /> : activeTab === 'shares' ? <MemberSharesPanel locale={locale} /> : activeTab === 'tokens' ? <MemberTokensPanel locale={locale} /> : activeTab === 'docs' ? <MemberDocsPanel locale={locale} /> : <MemberAccountPanel locale={locale} />}</section></div></main>;
}
