...
SELECT id, account_id, token_hash, created_at, expires_at, last_used_at, revoked_at
...
`, accountID, formatTime(s.now()))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&session.ID, &session.AccountID, &session.TokenHash, &createdAt, &expiresAt, &lastUsedAt, &revokedAt); err != nil {
			return nil, err
		}
