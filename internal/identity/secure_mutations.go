...
// CompletePasswordResetAndCreateSession clears password_reset_required with a
// CAS on the expected old hash, revokes every existing session, and issues the
// next session in one transaction. The new password hash must be computed
// outside the write transaction.
func (s *Service) CompletePasswordResetAndCreateSession(ctx context.Context, accountID, expectedOldHash, newPasswordHash string, purpose SessionPurpose, ttl time.Duration, auditWriter TxAuditWriter) (SessionToken, error) {
	if strings.TrimSpace(accountID) == "" || strings.TrimSpace(expectedOldHash) == "" || strings.TrimSpace(newPasswordHash) == "" {
		return SessionToken{}, ErrInvalidCredential
	}
	if !purpose.Valid() {
		return SessionToken{}, fieldError("purpose", "must be full or totp_enrollment")
	}
	if ttl <= 0 {
...
UPDATE accounts
SET password_hash = ?, password_reset_required = 0, updated_at = ?
WHERE id = ? AND status = 'active' AND password_hash = ? AND password_reset_required = 1
`, newPasswordHash, formatTime(now), accountID, expectedOldHash)
	if err != nil {
		return SessionToken{}, err
	}
	affected, err := result.RowsAffected()
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`, session.ID, session.AccountID, session.TokenHash, session.Entry, session.Purpose,
		session.CredentialGeneration, formatTime(session.CreatedAt), formatTime(session.ExpiresAt), formatTime(session.LastUsedAt)); err != nil {
		return SessionToken{}, err
	}
	if auditWriter != nil {
		if err := auditWriter(ctx, tx); err != nil {
