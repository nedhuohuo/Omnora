...
func newFixture(t *testing.T) *fixture {
	t.Helper()
	dbPath := filepath.Join(t.TempDir(), "mountadmin.db")
	db, err := store.OpenSQLite(context.Background(), store.SQLiteOptions{
		Path: dbPath, BusyTimeout: time.Second,
	})
	if err != nil {
...
func (f *fixture) peerService(t *testing.T) *mountadmin.Service {
	t.Helper()
	peer, err := store.OpenSQLite(context.Background(), store.SQLiteOptions{Path: f.dbPath, BusyTimeout: 3 * time.Second})
	if err != nil {
		t.Fatalf("open peer database: %v", err)
	}
	t.Cleanup(func() { _ = peer.Close() })
	return mountadmin.New(peer.SQL(), f.root)
}
...
func TestConcurrentCreateMountClaimsSameIdentityOnce(t *testing.T) {
	fixture := newFixture(t)
	services := []*mountadmin.Service{fixture.service, fixture.peerService(t)}
	root := filepath.Join(fixture.root, "same-root")
	if err := os.MkdirAll(root, 0o755); err != nil {
		t.Fatalf("create mount root: %v", err)
	}
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		go func() {
			defer workers.Done()
			<-start
			_, errorsByAttempt[attempt] = services[attempt%len(services)].CreateMount(context.Background(), fixture.initialAdmin.ID, mountadmin.CreateRequest{
				ID: fmt.Sprintf("mnt-same-%02d", attempt), DisplayName: fmt.Sprintf("Same %02d", attempt), RootPath: root,
				Governance: domain.MountGovernanceNormal,
				Mode:       domain.MountModeReadOnly,
			}, nil)
		}()
	...
	assertOneConcurrentMountCreated(t, errorsByAttempt)
	assertCount(t, fixture.db, `SELECT COUNT(*) FROM mount_identity_claims`, 2)
}

func TestConcurrentCreateMountRejectsParentChildRoots(t *testing.T) {
	fixture := newFixture(t)
	services := []*mountadmin.Service{fixture.service, fixture.peerService(t)}
	parent := filepath.Join(fixture.root, "tree")
	child := filepath.Join(parent, "child")
	if err := os.MkdirAll(child, 0o755); err != nil {
		t.Fatalf("create nested mount roots: %v", err)
	}
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		go func() {
			...
			_, errorsByAttempt[attempt] = services[attempt%len(services)].CreateMount(context.Background(), fixture.initialAdmin.ID, mountadmin.CreateRequest{
				ID: fmt.Sprintf("mnt-tree-%02d", attempt), DisplayName: fmt.Sprintf("Tree %02d", attempt), RootPath: root,
				Governance: domain.MountGovernanceNormal,
				Mode:       domain.MountModeReadOnly,
			}, nil)
		}()
...
func assertOneConcurrentMountCreated(t *testing.T, results []error) {
	t.Helper()
	created := 0
	for _, err := range results {
		...
		if !errors.Is(err, mountadmin.ErrMountConflict) {
			t.Fatalf("concurrent CreateMount error = %v, want ErrMountConflict", err)
		}
	}
	if created != 1 {
		t.Fatalf("concurrent CreateMount successes = %d, want 1", created)
	}
}
