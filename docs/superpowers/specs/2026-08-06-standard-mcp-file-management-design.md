...
> **迁移状态说明：** 本文记录迁移前已确认并实现的 Space 版 MCP 工具、scope 和协议交付。其 25 个工具、15 个 scope、`spaceId` 和空间 ACL 只用于当前实现回归，不是账号—挂载目标契约；目标资源联合类型、协作排除规则和迁移后 catalog 重算以[账号、挂载与内容授权设计](2026-08-08-account-mount-access-design.md)及[MCP 指南](../../mcp/README.md)为准。
