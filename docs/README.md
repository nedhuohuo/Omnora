...
当前业务模型为“唯一默认挂载中的个人目录 + 独立授权共用挂载 + 目录协作”，成员端产品展示名为“个人空间 / 团队空间 / 协作”，并采用受限挂载方案 B。仓库 Go、React、OpenAPI 和 MCP 已按该模型实现；OAuth 授权配置文件仍为 `NOT IMPLEMENTED`，资源与性能数值仍需按验收门槛验证。
