...
- SQLite WAL 数据层：保存账号、空间成员、挂载白名单授权、分享、Token、审计事件和轻量元数据。
- React 前端：通过统一 `/app` 工作台提供成员文件空间、账号安全、分享、AI Token 和管理员控制面；`/admin` 保留兼容路由。
...
- **空间与挂载：** 用空间组织个人文件、共享文件和 NAS 已有目录；首次初始化及后续启动会自动接入允许根下的 Docker 映射，并只向首个管理员的个人空间授予访问；其他挂载仍要求显式授权。
- **管理员控制面：** 仅系统管理员可添加或修改挂载，并逐账号维护纯白名单；管理员身份本身不授予内容访问。
- **文件管理：** 账号、空间成员权限与挂载白名单、文件浏览、大文件续传、托管挂载回收站与审计。
